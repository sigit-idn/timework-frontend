...
const Employee: React.FC<EmployeeProps> = ({ 
	employee: { id, name, position, tasks },
	isAdmin 
}) => {
	return (
		<tr>
							...
							{isAdmin && (
								<>
									<Link
										to={`/friends/${id}/edit`}
										state={{ name }}
										className="py-1 px-2 w-max bg-yellow-500 text-white rounded text-xs transition hover:bg-yellow-600 opacity-100 block md:opacity-0 group-hover:opacity-100"
									>
										Edit
									</Link>
									<Link
			...
			<td className="md:px-6 py-4 whitespace-no-wrap border-b border-gray-200">
				...
				<div className="flex text-indigo-500 text-center md:hidden items-center relative w-20 mt-2">
					<span className="flex-1 py-1 border border-indigo-500 rounded-l">
						{tasks?.filter(({ taskEnd }) => !taskEnd).length ?? 0}
					</span>
					<Link
						to={"/friends/tasks/" + id}
						state={{ name }}
